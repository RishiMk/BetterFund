...
export default function AdminDashboard() {
  ...
  const fetchSuccessStories = async () => {
    ...
    if (!token) return;
    try {
      const res = await axios.get('http://localhost:5245/api/successstories', {
  ...
  const handleDeleteStory = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5245/api/successstories/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const fetchPendingCampaigns = () => {
    ...
    axios
      .get('http://localhost:8081/api/campaign/admin/pending-campaigns', {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
  ...
  const fetchDashboardStats = () => {
    ...
    axios
      .get('http://localhost:8081/api/campaign/admin/dashboard-stats', {
        headers: { Authorization: `Bearer ${token}` },
      })
      .then((res) => {
        const d = res.data;
  ...
  const handleApproveCampaign = (notes) => {
    ...
    axios
      .put(
        `http://localhost:8081/api/admin/campaigns/${selectedCampaign.campaignId}/approve`,
        { notes },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      .then(() => {
  ...
  const handleRejectCampaign = (notes) => {
    ...
    axios
      .put(
        `http://localhost:8081/api/admin/campaigns/${selectedCampaign.campaignId}/reject`,
        { notes },
        { headers: { Authorization: `Bearer ${token}` } }
      )
      .then(() => {
  ...
  const handleCreateStory = async (e) => {
    ...
    try {
      await axios.post('http://localhost:5245/api/successstories', formData, {
        headers: { Authorization: `Bearer ${token}` },
      });
                  ...
                  {stories.map((s) => (
                    <tr key={s.successId}>
                      ...
                      <td style={cellStyle}>
                        {s.imageURL ? (
                          <img
                            src={`http://localhost:5245${s.imageURL}`}
                            alt="story"
                            style={{ height: 60 }}
                          />
                        ) : (
